...
{  
  ...
  //auto Nhits = step->GetNumberofHits();
  
  // step length and hit position
  G4double stepLength = 0.;
  G4ThreeVector hit_Position;
  G4double hit_Time = 0.;
  if ( step->GetTrack()->GetDefinition()->GetPDGCharge() != 0. ) {
    stepLength = step->GetStepLength();
    hit_Position = step->GetTrack()->GetPosition();
    hit_Time = step->GetTrack()->GetGlobalTime();
  }

  const double W_Ar = 26*10^(-6);
  int n_Hits = 0;
  if ( edep > 0) n_Hits++;  
  ...
  // Add values
  hit->Add(edep, stepLength, hit_Position, hit_Time, n_Hits);
  hitTotal->Add(edep, stepLength, hit_Position, hit_Time, n_Hits); 
